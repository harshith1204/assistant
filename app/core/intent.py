...
class ConversationalIntentDetector:
    ...
    async def detect_intent(
        ...
        """Detect intent from conversational message using LLM only"""
        
        # LLM-based deep understanding
        deep_analysis = await self._deep_intent_analysis(message, context)
        
        # Add message metadata
        deep_analysis["original_message"] = message
        deep_analysis["timestamp"] = datetime.now(timezone.utc).isoformat()
