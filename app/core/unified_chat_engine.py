"""Unified Chat Engine combining standard and enhanced conversational capabilities"""
...
class UnifiedChatEngine:
    """Unified chat engine with both standard and enhanced conversational capabilities"""
    
    def __init__(self):
        """Initialize unified chat engine"""
        ...
        # Action handlers mapping for conversational mode
    ...
    async def process_message(
        ...
        """Process a chat message with dual context (standard mode)"""
        try:
    ...
    async def stream_message(
        ...
        """Stream chat response (standard mode)"""
        try:
            # Ensure dual context
            conversation_id = request.conversation_id or str(uuid.uuid4())
            conversation_id, user_id = self.memory_manager.ensure_dual_context(
    ...
    # ==================== Enhanced Conversational Methods ====================
    ...
    async def process_conversational_message(
        ...
        """Process message conversationally with streaming updates (enhanced mode)"""
...
Provide 3 short, specific questions that would naturally continue the conversation."""
    ...
    async def _handle_chat_action(
        ...
        # Get context
        context = await self._prepare_context_simple(conversation, user_id)
...
Respond naturally and conversationally.""".format(
    ...
    async def _prepare_context_simple(
        self,
        conversation: Conversation,
        user_id: Optional[str]
    ) -> Dict[str, Any]:
        """Prepare simple context for processing"""
