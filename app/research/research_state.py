"""Business-specific state management for deep research integration"""
...
from app.research.state import AgentState, SupervisorState, ResearcherState
...
class BusinessResearchBrief(ResearchBrief):
    ...
    def add_financial_projections(self, projections: Dict[str, Any]):
        """Add financial projections to the research brief"""
        self.financial_projections = projections

    def set_implementation_roadmap(self, roadmap: List[Dict[str, Any]]):
        """Set implementation roadmap"""
        self.implementation_roadmap = roadmap

    def update_integration_status(self, crm_status: str = None, pms_status: str = None):
        """Update CRM/PMS integration status"""
        if crm_status:
            self.crm_integration_status = crm_status
        if pms_status:
            self.pms_integration_status = pms_status
...
class BusinessIdea(Idea):
    ...
    def generate_business_case_summary(self) -> str:
        """Generate a business case summary"""
        summary_parts = []

        summary_parts.append(f"**{self.idea}**")
        summary_parts.append(f"**Rationale:** {self.rationale}")

        if self.target_market_size:
            summary_parts.append(f"**Target Market Size:** {self.target_market_size:,}")

        if self.projected_revenue_year1:
            summary_parts.append(f"**Projected Revenue Year 1:** ${self.projected_revenue_year1:,.0f}")

        if self.time_to_market_months:
            summary_parts.append(f"**Time to Market:** {self.time_to_market_months} months")

        if self.prerequisites:
            summary_parts.append(f"**Prerequisites:** {', '.join(self.prerequisites)}")

        if self.risks:
            summary_parts.append(f"**Key Risks:** {', '.join(self.risks)}")
